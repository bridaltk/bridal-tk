...
*
*/
if (!function_exists('mona_init_vu_post')){
	function mona_init_vu_post(){		
		if( isset( $_GET['mona-support-param'] ) ){
			$id = (int) $_GET['mona-support-param'];
...
if (!function_exists('mona_get_featured_image_post')){
	function mona_get_featured_image_post( $post_id = false, $size = 'thumbnail', $return_default = true ){
		...
		if( $post_thumbnail_id > 0 ){
			$post_thumbnail_img = mona_get_featured_image( $post_thumbnail_id, $size, $return_default );
...
if (!function_exists('mona_get_featured_image_default')){
	function mona_get_featured_image_default( $size = 'thumbnail' ) {
		...
		if( $no_image == 0 ){
			...
			if( $size )
				return '//placehold.it/'.$size['width'].'x'.$size['height'];
			else
				return '//placehold.it/600x600';
		}else{
